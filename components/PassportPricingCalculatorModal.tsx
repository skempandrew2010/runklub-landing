...
// subscription tier's payout rate, then (optionally) applies the number
// they land on straight into the offer form that opened this.
export default function PassportPricingCalculatorModal({
  ...
  useEffect(() => {
    if (manuallySet || faceValue <= 0) return
    setCredits(suggestedCreditCost(faceValue))
  }, [faceValue, manuallySet])

  const { low, high } = calculatePayoutRange(credits)
  const showWarning = faceValue > 0 && low < faceValue * LOW_PAYOUT_WARNING_SHARE
  const breakdown = payoutBreakdown(credits, faceValue)
  const suggestion = faceValue > 0 ? suggestedCreditCost(faceValue) : null

  return (
    <ModalPortal>
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
        <div
          className="w-full sm:max-w-sm max-h-[85vh] overflow-y-auto bg-[#1e2d12] border border-[#2e3d1a] rounded-t-3xl sm:rounded-3xl p-5 pb-8 sm:pb-5 animate-[fadeUp_0.25s_ease-out_forwards]"
        ...
        >
          ...
          <div className="mb-5">
            ...
            <div className="flex rounded-full bg-[#1a2110] p-0.5 border border-[#2e3d1a] mb-3">
              <button
                onClick={() => setMode("flat")}
                className={`flex-1 py-1.5 rounded-full text-xs font-bold transition ${mode === "flat" ? "bg-[#c5f135] text-[#1a2110]" : "text-white/40 hover:text-white/70"}`}
              >
                I know my price
              </button>
              <button
                onClick={() => setMode("membership")}
                className={`flex-1 py-1.5 rounded-full text-xs font-bold transition ${mode === "membership" ? "bg-[#c5f135] text-[#1a2110]" : "text-white/40 hover:text-white/70"}`}
              >
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="15.00"
                    className="w-full bg-[#1a2110] border border-[#2e3d1a] rounded-xl pl-7 pr-3 py-2.5 text-sm text-white placeholder-white/25 focus:outline-none focus:border-[#c5f135]/50 transition"
                  />
                </div>
              </div>
            ) : (
