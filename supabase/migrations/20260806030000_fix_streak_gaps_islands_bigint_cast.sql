-- evaluate_checkin_challenges()'s "gaps and islands" streak grouping did
-- `d - (row_number() over (...) * 7)` where d is a date and row_number()
-- returns bigint — Postgres has no date - bigint operator, only
-- date - integer, so this raised "operator does not exist: date - bigint"
-- on every single run_checkins insert (the trigger runs in the same
-- transaction, so the whole check-in rolled back). Casting the row_number
-- product to int fixes both the overall and weekend-only streak CTEs.
