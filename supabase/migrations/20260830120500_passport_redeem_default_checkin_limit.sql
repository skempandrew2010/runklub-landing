-- Per-run check-in cap now falls back to the klub's default when the run
-- itself doesn't set one, same override pattern already used for credit value.
...
begin
  ...
  select passport_program_enrolled, passport_default_credit_value, passport_default_checkin_limit,
         passport_monthly_checkin_limit_per_user, passport_monthly_checkin_limit_total
    into v_enrolled, v_default_credit_value, v_default_checkin_limit,
         v_monthly_limit_per_user, v_monthly_limit_total
